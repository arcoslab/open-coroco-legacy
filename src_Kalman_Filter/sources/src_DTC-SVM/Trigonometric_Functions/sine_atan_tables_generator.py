...
for position in range(table_size):
  sine=math.sin(position *(90*math.pi/180)/(table_size-1) )
...
#####################################################################################################
#####################################################################################################
table_file = open('atan_table.h','w')
...
table_file.write ("#define ATAN_TABLE_SIZE   ")
table_file.write (repr(table_size))
table_file.write ("\n#define ATAN_TABLE_SIZE_F ")
table_file.write (repr(table_size-1))
table_file.write ("\n\nfloat atan_table [SIN_TABLE_SIZE] = ")
table_file.write ("\n{\n")


for position in range(table_size):
  atang=(180.0/math.pi)*math.atan(float(position)/float(table_size-1) )
  table_file.write (repr(atang))
  table_file.write ("f,\n")
  #print "sine: ",sine,"f,"
