...
if (center_aligned_state==FIRST_HALF)
{
  ...
  voltage_measure (ADC1,ADC_CHANNEL1); //isA shunt
}
else 
{
...
void adc_isr(void)
{
  ...
  if (adc_counter==0)
  {
    ...
    voltage_measure (ADC1,ADC_CHANNEL2); //isB shunt
    adc_counter++;
  }
  else if (adc_counter==1)
  {
    V_stm32_B = adc_read_regular(ADC1);
    voltage_measure (ADC1,ADC_CHANNEL15); //strain_gauge torque sensor
    adc_counter++; 
  }
  ...
  else if (adc_counter==2)
  {
    ...
    voltage_measure (ADC1,ADC_CHANNEL3); //Ud
    adc_counter++; 
  }
